...
_root.insert_child(_stack);
self.ui_main = _stack;

var _title = new UIText({}, {
    text_ref: function() { return "G.E.M.S."; },
    font: SPACE_FONT_BOLD,
    color: global.space_color.white,
    xscale: 1.6,
    yscale: 1.6,
});
_stack.insert_child(_title);

var _subtitle = new UIText({}, {
    text_ref: function() { return "Title"; },
    font: SPACE_FONT_NORMAL,
    color: global.space_color.grey,
});
_stack.insert_child(_subtitle);

var _menu = new UIElement({ width: "40%", gap: 12, flexDirection: "column" });
_stack.insert_child(_menu);
self.ui_menu = _menu;
...
var _start = space_ui_button("Start", "", function() {
    // No gameplay room yet; keep this as a clean hook point.
    // You can replace this with `room_goto(rm_game)` later.
});
var _options = space_ui_button("Options", "", function() { self.owner.show_settings(); });
_options.owner = self;
var _quit = space_ui_button("Quit", "", function() { game_end(); });

_menu.insert_child(_start);
_menu.insert_child(_options);
_menu.insert_child(_quit);

UIManager.insert(_root);
