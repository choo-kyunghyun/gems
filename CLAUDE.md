...
- **`SlotDrag`**: standalone static class (not a `UIComponent`) backing drag-and-drop between `UISlots` grids (built with `draggable: true`). Grids call `SlotDrag.begin(grid, i)` on press and `SlotDrag.hover(grid, j)` each frame the cursor is over a slot during a drag; `SlotDrag.draw()` in `Draw_75` (after `UI.draw`) draws the carried item and resolves on button-**up** — drop onto the reported hover slot (swaps occupants, works across grids), or return to source if over none. Resolving on the button-up *level* (not the release *edge*) avoids the drop racing the cancel, so a release that lands a frame off still drops.
