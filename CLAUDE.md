...
- **JS**: JavaScript is the main language — all game logic is JS, never GML; The `.d.js` stubs are autogenerated by the toolchain and not yet reliable — a stub is never authority: when one disagrees with real usage, check the manual (`gm-cli manual read`) and trust it;
...
- **Citations**: an upstream ticket is cited by tool + bare number (`gm-cli #231`, GMRT `#15095`) — **never** a GitHub issue/PR URL, and never the `owner/repo#n` form, in any file or commit message: both post a cross-reference into the other repository, and this project never links or pings one. Naming a repo in prose is fine and often necessary (`YoYoGames/GameMaker-Bugs` is the GMRT tracker — GMRT.md), as is a required license attribution URL (`README.md`'s font/model credits) or a plain URL to the doc that owns a fact (`manual.gamemaker.io`); only the issue-reference forms are out.
