...
- **`SlotDrag`**: standalone static class (not a `UIComponent`) backing drag-and-drop between `UISlots` grids (built with `draggable: true`). `SlotDrag.draw()` in `Draw_75` (after `UI.draw`) draws the carried item under the cursor and, when the button releases over no slot, returns it to its source. Grids call `SlotDrag.begin/drop` themselves; drop swaps occupants and works across grids.
