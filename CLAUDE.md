...
- **No class getters** — GMRT 0.19 does not reliably invoke a `get x()` accessor: the body never runs and the read yields `undefined` (confirmed via logging — the getter never fired). Use a plain method or inline the expression (see `UIStepper` inlining its can-step checks).
- **Guard `!(pos.width > 0)` before drawing a UI component** — on the first frame after a scene transition the flexpanel layout isn't computed yet, so `getLayoutPosition()` returns NaN width/height; drawing with NaN coords faults. Test `> 0`, not `<= 0` (`NaN <= 0` is `false`, so the naive guard misses it) — see `UIStepper.onUpdate`/`onDraw`.
...
- **`clipboard_has_text()` returns `false` even when the clipboard holds text** — don't gate paste on it. Read `clipboard_get_text()` directly and treat `""`/`undefined` as empty (see `UIInput._paste`).
- **Regex `.replace()` faults on the GMRT JS engine** — `str.replace(/.../g, …)` hard-faults. Strip/transform characters with explicit `charCodeAt`/index loops instead (see `UIInput._insert`/`_accept` rejecting control chars).
