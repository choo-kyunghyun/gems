...
- **Top-level bare `const` is not visible to other scripts** — share via `globalThis.Name`. Top-level `function` declarations *are* global (e.g. `gemsButton`, `teardownScene` in `demo.js`).
...
- **A class `static` method cannot call another `static` method of the same class** (`this._x()` *or* `ClassName._x()` from inside a `static` method hard-faults). Object-method→object-method via `this` is fine (`QuestLog.report`→`this._allMet`). Use a plain `globalThis.X = { … }` object for registries that call their own helpers (`QuestLog`, `Profile`, `InventorySystem`, `Achievement`); a class with static methods is OK only if none of them call a sibling static.
...
**GemsUI kit** (`scripts/demo/demo.js`): a themed factory library so scenes build UI declaratively instead of hand-wiring `UIElement`/`UIPanel`/`UIText`. All visual constants live in **`globalThis.GemsTheme`** (colors as hex strings, geometry as numbers); the `gems*` free functions parse + compose them. Containers: `gemsRoot(opts?)` (full-screen scene root), `gemsList(opts?)` (vertical stack), `gemsGrid(opts?)` (horizontal wrap row), `gemsPanel(opts?)`/`gemsCard(opts?)` (rounded panel; card adds gradient+border+shadow), `gemsHeader(title, opts?)` (title bar), `gemsSection(title, opts?)` (titled card with divider), `gemsRow(label, control, opts?)`, `gemsDivider(opts?)`. Widgets: `gemsLabel(label, opts?)`, `gemsButton(label, onClick, opts?)`, `gemsIconButton(sprite, onClick, opts?)`, `gemsToggle(label, getValue, onToggle, opts?)` (renders `label: ON/OFF`), `gemsSlider(key, min?, max?, step?)` + `gemsSelect(key, items)` (Settings-bound), `gemsSelectCustom(items, index, onChange)`. `label`/`onText`/`offText` accept a string or `() => string` (live `I18n.textRef`); color opts accept a `GemsTheme` key, hex string, or raw color int. Styling lives only in `GemsTheme` + `UIPanel` — `UIPanel` gained optional `color2` (vertical gradient), `border`/`borderColor`, and `shadow`/`shadowColor`/`shadowAlpha` (all default off, so existing callers are unchanged). **`teardownScene(scene)`** releases the `world`/`renderer`/`camera`/`ui` a scene holds on `this`, in dependency order (missing fields skipped) — call it from `destroy()` after releasing scene-specific resources (controllers, levels).
