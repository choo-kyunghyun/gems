...
- **RPG / gameplay layer** (Templates) — items & inventory (`Item`, `Inventory`, `Equipment`, `Consumable`, `Container`, `Encumbrance` + their systems), combat (`Health`, `Stats`, `MeleeSystem`, `ProjectileSystem`, `Enemy`/`SlimeAI`), progression (`QuestLog`, `NPC`, `Dialogue`), loot (`ItemDrop`, `Rarity`), and sprite animation (`Animator`/`AnimationSystem`). Content registries: shared `RpgContent` registers **all** items (weapons incl., `wood_sword` melee + `blaster` ranged) + rarities + recipes; the only per-genre content is `TopDownContent` (top-down quests + achievements). Both controllers honor a weapon's `melee` flag (swing vs fire), so one shared weapon def works in both. The **shared RPG layer the platformer + top-down templates both build on lives in `Templates/RPG`**: `RpgContent`, `RpgPlayer` (player spawn + the shared bullet preset + firing), `RpgScene` (combat/loot step helpers), `RpgInventoryUI` (the inventory/equipment window — the single place to overhaul the inventory UI), `RpgWorldOverlay` (world-space drops/bullets/reach-zone). Each genre scene/controller keeps only its movement, level, and flavor.
- **Genre UI** — `SystemMenu` (the app's single global tabbed overlay: pause + settings + system/sim controls + about + debug; opened by F1 anywhere or Esc/Start in gameplay, it pauses all sim), `StorageUI`, `BuildMode` (TopDown grid build mode — claim a buildable `Zone` at a Survey Post, then place/deconstruct wall + floor tiles snapped to the grid for wood), and the shared world-space overlay `RpgWorldOverlay` (item drops + bullets + the top-down reach zone, used by both RPG scenes); HUD + draggable inventory windows are real UI panels built by each scene and drawn on the GUI layer.
