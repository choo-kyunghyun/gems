...
- **RPG / gameplay layer** (Templates) — items & inventory (`Item`, `Inventory`, `Equipment`, `Consumable`, `Container`, `Encumbrance` + their systems), combat (`Health`, `Stats`, `MeleeSystem`, `ProjectileSystem`, `Enemy`/`SlimeAI`), progression (`QuestLog`, `NPC`, `Dialogue`), loot (`ItemDrop`, `Rarity`), and sprite animation (`Animator`/`AnimationSystem`). Content registries: shared `RpgContent` (rarities + universal items + recipes) layered under per-genre `PlatformerContent`/`TopDownContent` (genre weapons, unique items, quests/achievements). The **shared RPG layer the platformer + top-down templates both build on lives in `Templates/RPG`**: `RpgContent`, `RpgPlayer` (player spawn + bullet firing), `RpgScene` (combat/loot step helpers), `RpgInventoryUI` (the inventory/equipment window — the single place to overhaul the inventory UI), `RpgWorldOverlay` (world-space drops/bullets/reach-zone). Each genre scene/controller keeps only its movement, level, and flavor.
