...
- Verify Core with the tests, not a fresh probe: `GEMS_TEST=1 gm-cli run gems.yyp` runs every case the `test*` modules register with `Test` (the Core areas' checks and `perf.*` families, then `testGame`'s cases, then `testStress`'s scenarios), logs a `[CHECK]` line per case and a `[BENCH]` line per measure or sample, and exits, and `GEMS_TEST=<id prefix>` (`GEMS_TEST=collision.`) runs only the cases under that prefix; a Core change that a case could catch gets the case, a per-op cost claim gets its measure in the area module's `perf.*` case, and a Game asset's behaviour on the pinned runtime gets a `testGame` case, never a Core one.
