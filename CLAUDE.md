...
1. The default is no comment. A comment earns its line only by stating what the code cannot: an invariant, a unit, a coordinate space, a why — never what the code does, nor when or how a fact was established (Prose & Commits → Record). Short is not exempt: a one-line narration of the statement below it is the same defect as a paragraph of it — no such fact, no comment. Existing comments are grandfathered; tighten only code you are already touching.
