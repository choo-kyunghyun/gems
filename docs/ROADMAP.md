...
- **`ItemDrop` drifted from the slot shape it ferries**: its typedef types `mods` as `string[]` and `RpgCombat`'s drop/pickup default it to `[]`, where `InventorySlot.mods` is a map (`{}`) — a mod installed after a ground round-trip writes named keys onto an array; and `ammo`/`rounds` never ride a drop, so a loaded gun lands empty. Align the payload with `InventorySlot` and fix the `?? []` defaults.
- **`InventorySystem.addSlot`'s fungible fallback can duplicate items**: it returns `add(...) === 0`, so a partial add reports false while units were already added — a transfer caller keeping the source slot on false duplicates the moved part. Latent: every live caller (`TradeSystem`/`StorageUI`/`RpgCombat`) guards `isInstanced()` first. Return the leftover like `add`, or reject fungible slots outright.
- **Economy payouts ignore their own leftovers**: `TradeSystem.sell` pays via `InventorySystem.add` and discards the unfit remainder (a slot-starved bag silently loses coins), and `EquipmentSystem.loadAmmoSlot`'s ammo-swap refund does the same with chambered rounds. Surface the leftover — block the action or ground it as an `ItemDrop`.
- **Nine hand-rolled registries across the kit**: `Item`/`Manufacturer`/`Rarity`/`Recipe`/`FactionSystem`/`Status`/`QuestLog`/`SettlementComponent`/`InteractAction` each re-implement the string-keyed registry + insertion-order array (the #15095 index-loop idiom) with drifting member sets and split storage (Map vs plain object) — `Recipe` lacks `has`/`all`, `Rarity` alone carries a caller-less `import`/`export` pair and no file header. With inheritance off the table, consolidate via a composed registry helper or at least align the family and drop the dead pair.
- **Kit typedefs written as line comments are invisible**: `Favorites`, `Merchant`, `StatusEffects`, `Hunger`, `Thirst`, and `Drowsiness` carry their `@typedef` in `//` comments, so the checker never sees shapes the ECS invariant calls the type system. Convert to `/** */` blocks.
...
- **`QuestLog.complete` falls back to the wrong shape**: `rewards` is typed `Array<{itemId,qty}>` but the no-rewards fallback returns `{}` — a caller looping `.length` no-ops by accident. Return `[]`.
...
- **`RenderLighting`'s flicker runs on the wall clock**: the torch flicker phase advances on `current_time`, so a paused game keeps flickering and `Time.scale` never dilates it — against the clock-split invariant that world-space effects ride sim time (`Weather`/`FloatingText` freeze). Drive it from a sim-time accumulator (the `Weather.time()` pattern).
