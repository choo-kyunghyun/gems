...
- **`Core`** — the pure engine: ECS (`World`), levels/zones, renderer passes, the UI system, input, utilities. References only engine concepts.
...
- **ECS shape**: components are **string tokens** (`globalThis.Position = "Position"`) with the data shape defined at the call site; systems are **plain objects** `{ update(world) }` (or stateless objects with named query/resolve methods); each scene owns its `World` — there is no `WORLD` global, and `World` never auto-runs systems — the scene's `step()` dispatches them explicitly.
...
- **AABB convention**: every collision/geometry consumer derives world-space edges through `AABB.edges(pos, box)` / `AABB.of(world, id)` — never inline `pos.x + box.x` (the non-uniform BBox anchor lives in one place).
...
- **[ecs.md](architecture/ecs.md)** — the `World` API (create/remove/add/get/query/export), `EntitySnapshot`, `IdPool`, the component/system pattern snippets, fixed-rate simulation (`world.update()` ticks + `alpha`, `Pipeline`, `Time`), the built-in system table (`Gravity`/`Movement`/`Solid`/`Separation`/`Trigger`/`State`/`Lifetime`/`Interpolation`/`Pathfinding`), and the pathfinding flow (`PathRequest`→`PathResponse`, `PathFollow`, `NavGrid`).
