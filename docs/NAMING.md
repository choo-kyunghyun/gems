...
- A lowercase prefix marks a script that is not logic: `scene*` for a screen, whose one global is a factory function of the same name, `content*` for a data module whose whole body is defs handed to a registry, and `test*` for a case module whose whole body is cases handed to the test runner (`sceneTest`), named for the Core area or the tier it covers (`testCollision`, `testGame`, `testStress`). A module that decides something stays PascalCase even when it registers defs.
- A script family keeps its conventional affix (`Render*`, `*System`). `*System` marks a ticker — an object with `update(level)` and the accessor of its own derived entry — so a namespace of on-demand operations over a component (`Bag`, `Loadout`, `Crafting`, `Effects`, `Trade`, `Anim`) carries no affix.
- A derived entry (what a system mints on the level's own entity) is a class named for what it holds (`View`, `NavGrid`, `Rooms`), never for the system that owns it; the owner's accessor is that noun in lowercase (`PuppetSystem.colliders`, `CameraSystem.view`, `PathfindingSystem.nav`). Its token is the owner's `KEY`; an owner that also keeps a record there names the second token for what it is (`RUNTIME`, `MIRROR`).
...
- A shader is `sh<Name>`, a particle system `ps<Name>`, an animation curve `ac<Name>`, a tile set `ts<Name>`.
...
- A mesh (`.vox`/`.mesh` under `datafiles/meshes/`) follows the sprite rule with no kind prefix, since the directory and extension are the kind: the file and its `Mesh.model` string share one camelCase `<material><Object>[<Variant>]` name. The string reaches a save, so renaming a mesh bumps `SaveGame.VERSION`.
