...
- A script family keeps its conventional affix (`Render*`, `*System`). `*System` marks a ticker — an object with `update(level)` and the accessor of its own cache entry — so a namespace of on-demand operations over a component (`Bag`, `Loadout`, `Crafting`, `Effects`, `Trade`, `Rig`) carries no affix.
- A cache entry is a class named for what it holds (`Colliders`, `View`, `NavGrid`, `Rooms`), never for the system that owns it; the owner's accessor is that noun in lowercase (`SolidSystem.colliders`, `CameraSystem.view`, `PathfindingSystem.nav`).
