...
Casing follows word boundaries, acronyms included (`pixUiBox`, not `pixUIBox`).

- `family` names the CONSUMER that reads the asset — a closed set: `item` (bag icons — the subject names the ART, not an item id: a def names its sprite explicitly and several ids may share one, `pixItemRounds` for every caliber), `tex` (wall/floor face textures), `terrain` (dual-grid terrain sets), `tile` (autotile piece sets), `grass` (the sheets `RenderGrass` strews over a material's ground — standing clumps and clutter, flat decals), `ui` (widget chrome/glyphs), `fx` (particle art). A bare subject with no family tag is reserved for entity art (`spineHuman`) and the garments its doll wears (`pixHatRedBandana`, `pixShoeDarkBrown` — the garment kind leads, naming the slot it dresses).
...
- Game-data metadata (manufacturer/rarity/stats/tier) never enters a name — it lives on the def and reaches the player through UI; a brand string appears only when the art itself is branded (`pixItemVektWrench`).
- Sounds: `snd<Subject>[<Event>]` for SFX (`sndGunFire`, bare `sndCoin`), `mus<Track>` for music.

## Data Keys

An asset name is an IDENTIFIER — GameMaker binds it in global scope and code reads it bare, so it is cased like one. A data key is not: item ids, i18n keys, and anything a save file holds are strings the engine compares, and they stay lowercase snake_case (`lead_pipe`, `td_gather`). The line matters where the two meet — an item def carries its id as a string and its icon as a bare ref side by side (`id: "lead_pipe"`, `sprite: pixItemLeadPipe`), each in its own casing, and a sprite is never derived from an id. Renaming an asset is a rename; renaming a key is a migration.
