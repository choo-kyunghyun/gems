...
- A script family keeps its conventional affix (`Render*`, `*System`). `*System` marks a ticker — an object with `update(level)` — so a namespace of on-demand operations over a component (`Bag`, `Loadout`, `Crafting`) carries no affix.
