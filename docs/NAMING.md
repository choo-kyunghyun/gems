...
- A script family keeps its conventional affix (`Render*`, `*System`). `*System` marks a ticker — an object with `update(level)` and the accessor of its own derived entry — so a namespace of on-demand operations over a component (`Bag`, `Loadout`, `Crafting`, `Effects`, `Trade`, `Rig`) carries no affix.
