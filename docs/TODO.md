...
- Scene-owned module state — `BuildMode` and `Interactable` keep their state as `scene._build*` / `scene._inter*` fields the scene never declares, and `BuildMode.active` mirrors `scene._buildActive`; each `build()` returns the handle object that holds it, the way every `*UI` page already does (the contract at `Window`), so the scene declares one field apiece
