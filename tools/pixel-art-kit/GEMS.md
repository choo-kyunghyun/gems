...
| **GM naming** | `spr_<family>_<subject>[_<variant>]`, snake_case (rule in CLAUDE.md → Media Asset Naming): `spr_item_<item_id>` icons, `spr_wear_<garment>` overlays, `spr_tex_<material>` textures, `spr_terrain_<material>` tilesets; a **bare subject** (`spr_human`, `spr_rat`) is reserved for entity strips. |
