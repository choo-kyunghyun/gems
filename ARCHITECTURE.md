...
- `create(world, spawn)` — registers the keymap (`Input.bindAll`), spawns the player via `RpgPlayer.spawn` (+ genre components), returns a plain `ctrl` state bag (`{ id, facing, ... }`). The `"bullet"` `EntityPreset` is now **shared** — `RpgPlayer.spawn` registers it once (kinematic + bbox-less so `GravitySystem` skips it and it can't be a raycast self-target; a no-op for gravity in top-down, which has none).
...
- `Weapon` (`{ damage, fireCd?, bulletSpeed?, melee?, reach? }`) — attack profile. A `melee` weapon swings a hitbox `reach` px in the facing direction (`MeleeSystem`); a ranged weapon fires a cursor-aimed bullet (`ProjectileSystem`). **Both controllers honor the `melee` flag** (platformer's `attack`, top-down's fire path), so the same weapon behaves the same in either genre.
...
`Rarity` (`scripts/Rarity/`) is a parallel registry of tiers (`{ id, name, color, valueMod }`); rarity color drives loot/inventory tinting. **`RpgContent`** (`scripts/RpgContent/`, the shared Templates/RPG layer) registers **all** content shared by both genres — the rarity tiers, the **full item set** (consumables, weapons `wood_sword` (melee) + `blaster` (ranged), armor/trinket/backpack, currency `coin`/`gem`/`key`, crafting materials), and the workbench recipes. The only per-genre content left is **`TopDownContent`** — top-down's quests + achievements (the platformer has none, so there's no `PlatformerContent`). Both scenes call `RpgContent.register()` once from `create()` (top-down via `TopDownContent.register()`, which calls it first) — **not** at top level, to dodge GMRT load-order issues. Because a weapon's `melee` flag is honored by both controllers, a single shared `wood_sword`/`blaster` def works identically in either genre (no more same-id divergence).
...
- `BuildMode` — **TopDown grid build mode** (stateless singleton, same module shape as `Interactable`). `build(scene)` once in `create()`, `update(scene)` each frame after `Interactable.update` (outside the tick loop), `drawWorld(scene)` in the scene's `draw()`. `B` toggles; the mouse wheel cycles a two-entry palette (wall = solid + collider, floor = walkable cosmetic), LMB places (1 wood, via `SlotDrag.pressed` — the already-latched LMB edge) and RMB deconstructs (refunds the wood). Building is gated to a **claimed buildable `Zone`**: the player presses `E` at a **Survey Post** (a `Station` of kind `"claim"`; `Interactable._open` routes that kind to `BuildMode.claim`, which `paintRect`s the `"buildable"` `ZoneMap` channel around the post). Wall edits set the tile in the level's wall `TileLayer` and call `TopDownLevel.remeshWalls` (re-greedy-mesh all wall colliders, so built walls re-merge seam-free); floor edits touch no collider. All per-scene state lives on the scene (`_built` = `"gx,gy" → palette`, `_buildPalette`, `_buildActive`), except a static `BuildMode.active` mirror that `TopDownController` reads to suppress weapon fire while building. **Tile occupancy is a truthy test** (`wallLayer.get(x,y)`), not `!== undefined` — `Grid.get` returns `0` for an empty in-bounds cell.
- `RpgWorldOverlay` — one shared **world-space** overlay (`drawWorld(scene)`, called from each RPG scene's own `draw()` inside the camera view) for both genres: item-drop rarity squares, bullets, and — only when the scene exposes `reachZone` (top-down) — the reach-quest zone. Replaced the near-identical per-genre `PlatformerUI`/`TopDownUI`; `_rarityColor` is shared with the scenes' inventory rows. The HUD, dialogue box, and inventory window are real UI panels the scene builds and the UI manager draws on the GUI layer (`Draw_75`). **TopDown's walls are a real tilemap** (`TopDownLevel` keeps a wall + floor `TileLayer` on the `Level`), drawn by a `RenderDebugTileMap` pass with the buildable region drawn by a `RenderZone` pass — both world-space renderer passes under the entity passes.

The duplicated combat/loot plumbing both RPG scenes carried is centralized in **`RpgScene`** (`scripts/RpgScene/`, Templates/RPG) — free functions taking the scene (composition; no class inheritance on GMRT): `trackDamage(scene, yOffset)`, `resolveDeaths(scene, { onKill?, spill? })`, `spillLoot`/`spawnDrop`, `collectDrops(scene, onCollect?)`, `checkPlayerDeath(scene, onRespawn)`. Genre-specific side effects (quest/profile counters, the respawn action, FloatingText height) are the callbacks/options; the scene supplies the contract fields `world`/`ctrl`/`enemies`/`_hpTrack`/`_invDirty`.

The draggable **inventory/equipment/stats window** is likewise centralized in **`RpgInventoryUI`** (`scripts/RpgInventoryUI/`, Templates/RPG): `build(scene)` creates the (hidden) `gemsWindow` (stored as `scene._invWin`), `rebuild(scene, { equipSlots, extraRows? })` repopulates its body from the live `Inventory`/`Equipment`/`Stats` — `equipSlots` is the equipment rows to show (top-down adds `backpack`), `extraRows(scene, body)` appends genre lines (top-down's Profile records). The scene keeps only the open/close state (`invOpen`, toggled on the `inventory` key) and calls `build` in `create()` + `rebuild` when `_invDirty`. This is intentionally the **one place** to overhaul into a table/grid-based inventory later — the current row-of-buttons layout is a placeholder. (The HUD stays per-scene — `_buildHud` differs: platformer = level/HP, top-down adds the quest tracker.)

`scenePlatformer.step()` is the reference orchestration: edge-toggle the inventory window → `controller.pollInput` → per tick: `snapshot` → `controller.update` → `physics` Pipeline → `EnemySystem` → `controller.attack` (melee/ranged) → `EnemySystem.resolveTouch`/spike damage → void-respawn → `RpgScene.trackDamage` → `RpgScene.resolveDeaths` → `RpgScene.collectDrops` → `RpgScene.checkPlayerDeath` → `flush`. (No pause gate in `step()` — `obj_game` skips `scene.step()` entirely while the `SystemMenu` overlay is open.) Multi-level scenes use an `_initLevel(index)` / `loadLevel(index)` pattern (rebuild `world`/level/controller/pipeline/renderer/camera per level); cumulative score persists across levels on the scene.
