...
The duplicated combat/loot plumbing both RPG scenes carried is centralized in **`RpgScene`** (`scripts/RpgScene/`, top-level RPG) — free functions taking the scene (composition; no class inheritance on GMRT): `trackDamage(scene, yOffset)`, `resolveDeaths(scene, { onKill?, spill? })`, `spillLoot`/`spawnDrop`, `collectDrops(scene, onCollect?)`, `checkPlayerDeath(scene, onRespawn)`. Genre-specific side effects (quest/profile counters, the respawn action, FloatingText height) are the callbacks/options; the scene supplies the contract fields `world`/`ctrl`/`enemies`/`_hpTrack`/`_invDirty`.
