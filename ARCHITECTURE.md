...
- `PlatformerUI`/`TopDownUI` — thin **world-space** overlays (`drawWorld(scene)`, called from the scene's own `draw()` inside the camera view): item-drop rarity squares, bullets, the reach-quest zone, and (platformer only) walls. The HUD, dialogue box, and inventory window are real UI panels the scene builds and the UI manager draws on the GUI layer (`Draw_75`) — they no longer live here. **TopDown's walls are now a real tilemap** (`TopDownLevel` keeps a wall + floor `TileLayer` on the `Level`), drawn by a `RenderDebugTileMap` pass with the buildable region drawn by a `RenderZone` pass — both world-space renderer passes under the entity passes, not in `TopDownUI`.
