...
The Core systems above are genre-agnostic. A playable genre scene layers a **controller** plus **gameplay systems** on top — these live in the genre's folder (`Platformer`/`RTS`/`Map` under **Templates**; **TopDown** and the shared **RPG** layer are top-level) and are orchestrated by the scene's `step()`, not auto-run by `World`.
...
**Genre UI managers** (spread across Core / RPG / TopDown / Templates) — gameplay overlays that are not the GemsUI scene panels:
