...
4. ~~`Mesh` component + `RenderMesh` pass (as `Volume`/`RenderVolume`)~~ — DONE 2026-07-04 (Core/Component + Core/Render, inserted in `RpgMap.build` for pitched maps; occlusion verified in-game: player in front / behind bench / behind deep table, all per-pixel correct with placeholder colored faces).
   ...
   - **Mesh lighting DONE 2026-07-05** (the "3D lighting instead of outlines" decision): `.vbuf` now carries UNSHADED albedo + a packed per-face normal (in the texcoord slot, stride unchanged); **`sh_meshlit`** lights it live — directional sun from **`WorldClock.sunDir()`** (east→west arc, strength 0 at night, warm dawn/dusk; injected via `RenderMesh`'s `opt.sun` like RenderLighting's ambient) + up to 8 nearest `Light` entities as point lights (RenderLighting's flicker formula, so mesh response tracks the glow pools). Verified in-game: day-course shading swing + at night a lantern lights the bed/cot FRONTS while tops stay dark. Composes under the light map (shader = direction, light map = absolute darkness). Uniform arrays (`shader_set_uniform_f_array`) verified on GMRT 0.20 (JS arrays bridge fine — probed per-stage with a diagnostic shader). Two tuning rules earned by the "black rock in the lantern pool" bug: the point term is **wrap-lit** (`POINT_FILL` 0.4 — a hard N·L zeroes every visible face when the light is behind the mesh, since only top+south faces exist) with **linear falloff** (matching `RenderLighting`'s pool gradient, not `atten²`), and **ambient = the sun's complement** (`1 − 0.9·strength`: 0.55 at noon, 1.0 at night) — a constant ambient double-darkened meshes at night vs the map-lit ground/sprites around them. Next lighting steps if wanted: sprite-side sun scalar (the upright-paper spike), wall meshes joining the lit pass.
