...
// Stats.maxStamina caps the pool.
globalThis.Endurance = {
  ...
  sprint(entities, id, wantSprint) {
    ...
    if (sta.value < max) {
      sta.value += Endurance.REGEN * dt;
      if (sta.value > max) sta.value = max;
    }
    if (sta.exhausted && sta.value >= max * Endurance.RECOVER) sta.exhausted = false;
    return false;
  },
};
