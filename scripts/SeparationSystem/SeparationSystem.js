 ...
 * moves that far through `move_and_collide` against `Solid`, so a push never lands a body inside
 * a wall. Each side pushes itself, so a pair separates by its whole overlap; a solid-off body
 * wears the empty mask, so it neither lists nor is pushed. Position is read back off the
 * instance (the components stay the truth — PuppetSystem).
 */
