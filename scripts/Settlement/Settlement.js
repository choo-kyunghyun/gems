 ...
 * the owner faction, a display name, a tint, and a SettlementComponent id array — is level-scope
 * scene state (`scene.settlement`), parked with the map's bundle and saved in its manifest entry,
 * the way `meta.climate` pins the sky. A level carries at most one, authored by `meta.settlement`
 * (ColonyMap._buildWorld — a faction hub, a raider camp) or founded at a Survey Post
 * (BuildMode.claim); until then it is unsettled.
 *
 * Free functions over the scene (composition; GMRT has no usable class inheritance) holding NO
...
globalThis.Settlement = {
   ...
   */
  found(scene, opt = {}) {
    if (scene.settlement !== undefined) return undefined;
    const s = {
