...
globalThis.UIDraw = {
   ...
   */
  arrowPair(pos, leftCol, rightCol) {
    const cy = pos.top + pos.height * 0.5;
    const pad = 14;
    const ah = 5;
    UIDraw.arrow(pos.left + pad + ah, cy, "left", ah, leftCol);
    UIDraw.arrow(pos.left + pos.width - pad - ah, cy, "right", ah, rightCol);
    return cy;
  },
   ...
   */
  outline(x1, y1, x2, y2, rad, col, thick) {
   ...
   */
  panel(x1, y1, x2, y2, rad, style, a = 1) {
   ...
   */
  bar(x1, y1, x2, y2, rad, fillTo, track, fill, borderOver) {
   ...
   */
  contain(sw, sh, x, y, w, h, maxScale = 0) {
