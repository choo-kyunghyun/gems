...
/**
 * GMRT: edge queries read once per frame; ring pulse uses Time.raw; no Map/Set iteration, no cached
 * primitive bool.
 */
globalThis.UINav = {
  focused: null,
  ...
  update() {
    ...
    if (inp.confirm) {
      ...
      if (comp !== null) {
        Audio.play({ sound: snd_button_click }); // cue before activate (may swap level)
    ...
    if (UINav.focused !== prevFocus) Audio.play({ sound: snd_button_muted });
  },
