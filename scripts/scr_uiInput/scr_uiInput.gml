function UIInput(_style = {}, _input = {}, _panel = {}, _text = {}, _placeholder = {}) : UIElement(_style) constructor {
    // Value
    ...
    // TODO: Proper caret index + selection (currently keyboard_string driven)
    self.caret_index = 0;
    ...
    // Background panel (composition, not inheritance)
    self.panel = new UIPanel({ width: "100%", height: "100%", position: "absolute" }, _panel);
    self.insert_child(self.panel);

    // Tooltip (only shown when content overflows)
    self.tooltip = new UITooltip({ width: "100%", height: "100%", position: "absolute" }, {
        text_ref: method(self, function() { return self.value; })
    });
    ...
    // Caret
    self.caret = new UIPanel({ width: _input[$ "caret_width"] ?? 3, height: "100%" }, {
        color: _input[$ "caret_color"] ?? #121212,
        alpha: 0,
        rad: 0
    });
    ...
    static on_update = function() {
        ...
        if (self.focus) {
            ...
            if (!_prev_focus) keyboard_string = self.value;
            self.paste_value();
